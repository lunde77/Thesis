...
for i=1:1
    ...
    global results[i,1], results[i,2],results[i,3:6], results[i,7:9],results[i,10:11],results[i,12], results[i,13],results[i,14], results[i,15], Overbid_distribution_out[:,i], Upwards_bids[:,i], Downwards_bids[:,i], results[i+1,1], results[i+1,2], results[i+1,3:6],results[i+1,7:9], results[i+1,10:11], results[i+1,12], results[i+1,13],results[i+1,14], results[i+1,15], Overbid_distribution_out[:,i+1] = Main_stochastic_CC_OSS(CB_Is, 3, 11, "hourly")  # actually minute to minute
    ...
    CSV.write("$base_path"*"3. Simulations\\Stochastic results\\Results_MR_Also.csv", results_df)

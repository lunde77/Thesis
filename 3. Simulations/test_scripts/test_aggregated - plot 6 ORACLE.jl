...
for x in z
    ...
    global magnitudes_overbids_out = zeros(525600,TE)

    for q=start:TE
        CB_Is = collect((q-1)*N_size[x]+1:q*N_size[x])


        global results[q,1], results[q,2], results[q,3:6], results[q,7:9], results[q,10:11], results[q,12], results[q,13], results[q,14], results[q,15], Overbid_distribution[:,q], Upwards_bids[:,(q-1)*NF+1:q*NF], Downwards_bids[:,(q-1)*NF+1:q*NF], xxx, magnitudes_overbids_out[:,q], hourly_revenue[:,q], hourly_penalty[:,q] = Main_Oracle(CB_Is)


        results_df = DataFrame(results, :auto)
        CSV.write("$base_path"*"3. Simulations\\Stochastic results\\results_N_bundles_Oracle_test_$(N_size[x]).csv", results_df)

        results_df = DataFrame(Overbid_distribution, :auto)
        CSV.write("$base_path"*"3. Simulations\\Stochastic results\\Overbid_distribution_N_bundles_Oracle_test_$(N_size[x]).csv", results_df)

        results_df = DataFrame(Upwards_bids, :auto)
        CSV.write("$base_path"*"3. Simulations\\Stochastic results\\Upwards_bids_N_bundles_Oracle_test_$(N_size[x]).csv", results_df)

        results_df = DataFrame(Downwards_bids, :auto)
        CSV.write("$base_path"*"3. Simulations\\Stochastic results\\Downwards_bids_N_bundles_Oracle_test_$(N_size[x]).csv", results_df)

        results_df = DataFrame(hourly_penalty, :auto)
        CSV.write("$base_path"*"3. Simulations\\Stochastic results\\Hourly_penalty_N_bundles_Oracle_test_$(N_size[x]).csv", results_df)

        results_df = DataFrame(hourly_revenue, :auto)
        CSV.write("$base_path"*"3. Simulations\\Stochastic results\\Hourly_revenue_N_bundles_Oracle_test_$(N_size[x]).csv", results_df)

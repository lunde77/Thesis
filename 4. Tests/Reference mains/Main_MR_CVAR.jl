...
# total_delivery_missed: How much of the total activation do we on daily average not deliverS
...
function Main_stochastic_CVAR_OSS(CB_Is, S_method, N_s, model_res)
    ...
    return revenue[1], penalty[1], total_cap_missed[1,:], average_cap_missed[1,:], total_delivery_missed[1,:], pr_flex_used_up[1], pr_flex_used_do[1], model_runtime, clock, missing_capacity_storer_per[1,:,:,:],  missing_capacity_storer[1,:,4], Up_bids_A[:,1], Do_bids_A[:,1]
end

...
function Main_stochastic_CC_OSS_folded_LER(CB_Is, model_res, P_rule)
    ...
    for w=1:NF
        ...
        for i=1:1
            ...
            if model_res == "hourly"
                global C_do, C_up, model_runtime = ALSO_X_hourly_LER(total_flex_up_s, total_flex_do_s, res_20_s)
        ...
        Threads.@threads for j=1:n_days
            ...
            revenue[1+(Day-1)*24:Day*24,1], penalty[1+(Day-1)*24:Day*24,1], missing_delivery_storer[1,Day,:], missing_capacity_storer[1,Day,:], missing_capacity_storer_per[1,Day,:, :], Freq_overbid_h[Day,:] = operation_LER(total_flex_up_r, total_flex_do_r, res_20_r, Ac_do_M_r, Ac_up_M_r, Do_bids_A[:,w], Up_bids_A[:,w], La_do_r, La_up_r)
    ...
    pr_flex_used_up[1] = round( (sum( Up_bids_A )*n_days)/sum(Total_flex_up[1,:,1:363] ), digits= 3 )  # % of upwards flexibity bid into market
    pr_flex_used_do[1] = round( (sum( Do_bids_A )*n_days)/sum(Total_flex_do[1,:,1:363] ), digits= 3 )  # % of downwards flexibity bid into market

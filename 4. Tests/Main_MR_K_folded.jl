...
function Main_stochastic_CC_OSS_folded(CB_Is, model_res, Sampling_in)
    ...
    ###### load the folds for in and out of sample days ######
    ...
    for w=1:NF
        ...
        for i=1:1
            ...
            ###### solve the model in a decomposed matter by appliying the Also-x method ######
            if model_res == "hourly"
        ...
        Threads.@threads for j=1:n_days # itterate over all days in fold OSS
            Day = OOS_days[w,j]
            println("day is $Day")

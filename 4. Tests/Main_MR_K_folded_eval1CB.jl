...
# CB1_pr_flex_used_up: amount of CB1 flexibity upwards used
# CB1_pr_flex_used_down: amount of CB1 flexibity downwards used
# CB_minute_do_flex: amount of CB1 flexibity downwards used every minute
# CB_minute_up_flex: amount of CB1 flexibity upwards used every minute 

function Main_stochastic_CC_OSS_folded_one(CB_Is, model_res, Sampling_in)
    ...
    global Sampling = Sampling_in                           # sample choise
    global q_epilon = 0.00001                               # termination of ALSO-X
    global fail_rate = 0.1                                  # P10 fail rate
    ...
    for w=1:NF
        ...
        Threads.@threads for j=1:n_days
            Day = OOS_days[w,j]
            println("day is $Day")
            # if sampling of day's ch0ose bid accordinly
            if Sampling == 3
                if Day ∈ D_w
                    b =  w
                else
            ...
            revenue[1+(Day-1)*24:Day*24,1], penalty[1+(Day-1)*24:Day*24,1], missing_delivery_storer[1,Day,:], missing_capacity_storer[1,Day,:], missing_capacity_storer_per[1,Day,:, :]  = operation(total_flex_up_r, total_flex_do_r, res_20_r, Ac_do_M_r, Ac_up_M_r, Do_bids_A[:,b], Up_bids_A[:,b], La_do_r, La_up_r)

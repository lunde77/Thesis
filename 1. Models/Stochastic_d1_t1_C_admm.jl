...
function Stochastic_d1_model_hourly(La_do, La_up, Ac_do, Ac_up, Power_rate, po_cap, kWh_cap, Power, Connected, SoC_start, SoC_A_cap, flex_up, flex_do, total_flex_up, total_flex_do, I, S, t, RM, gamma, lambda, up_input, do_input)
   ...
   Pen_e_coef = 0 # multiplier on energy for not delivering the activation -> 6, implies we have to pay the capacity back and that it 5 times as expensive tp buy the capacity back
   ...
   ### Obejective ###
   @objective(Mo, Min,  -Income+Penalty+gamma[1]/2*( (sum(per_dev_up[m,s] for m=1:M, s=1:S)+up_input)/(M_d*S)-1+slack_up+lambda[1])^2+gamma[2]/2*( (sum(per_dev_do[m,s] for m=1:M, s=1:S )+do_input)/(M_d*S)+slack_do-1+lambda[2])^2  )

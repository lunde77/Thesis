...
function Stochastic_chancer_model_hourly(total_flex_do, total_flex_up, total_res_20, Y_else, q, lambda, gamma)
   ...
   M_e  = findmax(total_res_20[1:60])[1]+1000
   ...
   @variable(Mo, 0 <= Slack)                    # slack varible allowing for more bids
   ...
   ### Obejective ###
   @objective(Mo, Min,  -(Capacity) +lambda*( sum(Y[m,s] for s=1:S, m=1:M)) +  (gamma)*( (sum(Y[m,s] for s=1:S, m=1:M)+Y_else)-q_obj+Slack)^2   ) ###

   @constraint(Mo, Capacity <= 10000)
   ...
   #@constraint(Mo, Con, sum(Y[m,s] for s=1:S, m=1:M) + Slack  == q )
   @constraint(Mo, Con, q_obj == q )
   ...
   return Mo, Con, Y, C_do, C_up, Slack
end

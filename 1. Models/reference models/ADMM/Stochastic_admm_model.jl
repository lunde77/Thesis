...
function Stochastic_admm_model(La_do, La_up, Ac_do, Ac_up, Power_rate, po_cap, kWh_cap, Power, Connected, SoC_start, SoC_A_cap, flex_up, flex_do, total_flex_up, total_flex_do, I, S, t, RM, gamma, lambda, up_input, do_input)
   ...
   @constraint(Mo, [m=2:M, i=1:I_o, s=1:S], Ma[m,i,s] <= (kWh_cap[(t-1)*60+m-1,i,s]/(po_cap[(t-1)*60+m-1,i,s]+0.00001)/RM-SoC[m-1,i,s] )*60*fixer_cons_1[m,i,s] )


   @constraint(Mo, [m=2:M, i=1:I_o, s=1:S], SoC[m,i,s] <=  Connected[(t-1)*60+m,i,s]*25 )

   @constraint(Mo, [i=I_o, s=1:S], SoC[1, i, s] ==  kWh_cap[(t-1)*60+1,i,s]/(po_cap[(t-1)*60+1,i,s]+0.00001)*Connected[(t-1)*60+1,i,s]+Po[1,i,s]/60 )  #### this has to be changed to SoC_start, as we're for now not reffering to the correct SoC, it should be m-1

   @constraint(Mo, [m=2:M, i=1:I_o, s=1:S], SoC[m,i,s] == (SoC[m-1,i,s]+Po[m,i,s]/60)*Connected[(t-1)*60+m,i,s] )

   @constraint(Mo, [m=2:M, i=1:I_o, s=1:S],  SoC[m,i,s] <= kWh_cap[(t-1)*60+m,i,s]/(po_cap[(t-1)*60+m,i,s]+0.00001)/RM*Connected[(t-1)*60+m,i,s] )


   #************************************************************************

   n_vars = JuMP.num_variables(Mo)

   n_linear_constraints = JuMP.num_constraints(Mo, JuMP.VariableRef, MOI.LessThan{Float64})


   println("The number of variables are $n_vars" )

   println("The number of constraints are $n_linear_constraints" )

...
function Stochastic_d1_model(La_do, La_up, Ac_do, Ac_up, Power_rate, po_cap, kWh_cap, Power, Connected, SoC_start, SoC_A_cap, I)
   ...
   @variable(Mo, 0 <= C_up_I[1:M_d, 1:I, s=1:S])     # Upwards bid placed on CB I - kW
   @variable(Mo, 0 <= C_do_I[1:M_d, 1:I, s=1:S])     # Downwards bid  on CB I  - kW
   ...
   @variable(Mo, 0 <= C_up_A[1:M_d, s=1:S])        # aggregator Upwards bid for scenario s - kW
   @variable(Mo, 0 <= C_do_A[1:M_d, s=1:S])        # aggregator Downwards bid for scenario s  - kW
   @variable(Mo, 0 <= Ma_A[1:M_d, s=1:S])          # aggregator Max power - kW
   @variable(Mo, 0 <= SoC_A[1:M_d, s=1:S])         # aggregator SoC for two scenarios, s=1 no activation, s=2 for simulated activation
   ...
   @constraint(Mo, [m=1:M_d, s=1:S], C_up[m] <= C_up_A[m,s] )
   @constraint(Mo, [m=1:M_d, s=1:S], C_do[m] <= C_do_A[m,s] )
   ...
   @constraint(Mo, [m=1:M_d, s=1:S], C_up_A[m,s] == sum(C_up_I[m,i,s] for i=1:I) )                                   # The upwards bid must be distributed over the charge boxses
   @constraint(Mo, [m=1:M_d, s=1:S], C_do_A[m,s] == sum(C_do_I[m,i,s] for i=1:I) )                                   # The downwards bid must be distributed over the charge boxses
   ...
   @constraint(Mo, [m=1:M_d, s=1:S], Ma_A[m,s]-Power_A[m,s] >= C_do_A[m,s] )                                           # The upwards flexibility has to be greater than the the the bids regulation
   @constraint(Mo, [m=1:M_d, s=1:S], Power_A[m,s] >= C_up_A[m,s]*0.2+C_do_A[m,s] )                                     # The downwards flexibility has to be greater than the the the bids regulation, hence there is delay factor added on
   @constraint(Mo, [m=1:M_d, s=1:S], (SoC_A_cap[m,s]-SoC_A[m,s]) >= C_do_A[m,s]*(1/3) )                              # There must at all times be enough resovior to be activated for 20 minutes of the - this is a

   # Flexibility constraint
   @constraint(Mo, [m=1:M_d, i=1:I, s=1:S], Ma[m,i,s]-Power[m,i,s] >= C_do_I[m,i,s] )                                    # The upwards flexibility has to be greater than the the the bids regulation
   @constraint(Mo, [m=1:M_d, i=1:I, s=1:S], Power[m,i,s] >= C_up_I[m,i,s] )                                            # The downwards flexibility has to be greater than the the the bids regulation
   ...
   # Power constraint
   @constraint(Mo, [m=1:M_d, i=1:I, s=1:S], Po[m,i,s] == Power[m,i,s]- Ac_up[m,s]*C_up_I[m,i,s]+Ac_do[m,s]*C_do_I[m,i,s])        # The power is the baseline + the activation power
   ...
   # Bid cosntraints
   @constraint(Mo, [m=1:M_d, i=1:I, s=1:S],  C_up_I[m,i,s] <= Connected[m,i,s]*M )                                     # Only bid when charger is Connected
   @constraint(Mo, [m=1:M_d, i=1:I, s=1:S],  C_do_I[m,i,s] <= Connected[m,i,s]*M )                                     # Only bid when charger is Connected

   @constraint(Mo, [m=2:M, t=1:T, s=1:S], C_up_A[(t-1)*60+1,s] == C_up_A[(t-1)*60+m,s] )                             # Bid must equal for all minutes in a hour
   @constraint(Mo, [m=2:M, t=1:T, s=1:S], C_do_A[(t-1)*60+1,s] == C_do_A[(t-1)*60+m,s] )                             # Bid must equal for all minutes in a hour
   ...
   return value.(C_up), value.(C_do), value.(C_up_I), value.(C_do_I), value.(Power_A), value.(Ma_A), value.(SoC_A), objective_value(Mo)
end

...
### output:
# rev:                          The revenue generated from bid quntities
# pen:                          The cost of not meetning the acatvation
...
# Missing_capacity_storer_per:  Overbid in % for all minutes - up, down and energy 144x3
# M_C_B:                        Frequency of overbid for each hour [24]


function operation(Total_flex_up, Total_flex_do, res_20_r, ac_do_m, ac_up_m, C_do, C_up, La_do, La_up)


    M_d = 1440  # number of minnuts in a day
    ...
    epsilon = 0.00001                           # adjuster for numerical errors


    for m=1:M_d # analyze each minute

        # find potential flexibility issues, as wee need to have enough flexibility in both direction concerning the activation level


        if Total_flex_do[m] < C_do[m]- epsilon # asses if  bid to much downwards flexibility
            Missing_capacity_storer[m,1] = 1                                         # if so added is an overbid
            Missing_capacity_storer_per[m,1] = (C_do[m]-Total_flex_do[m])/C_do[m]    # find magnitude of overbid in %
        end
        if Total_flex_up[m] < C_up[m]+C_do[m]*0.2 - epsilon # asses if  bid to much upwards flexibility
            Missing_capacity_storer[m,2] = 1
            Missing_capacity_storer_per[m,2] = (( C_up[m]+C_do[m]*0.2)-Total_flex_up[m])/( C_up[m]+C_do[m]*0.2)
        end
        if res_20_r[m] < C_do[m]                           # asses if  bid to much energy flexibility
            Missing_capacity_storer[m,3] = 1- epsilon
        ...
        if sum(Missing_capacity_storer[m,:]) > 0.0000001 # if any capacity is violated
            Missing_capacity_storer[m,4] = 1               # calcuate all frequency of overbids
        end

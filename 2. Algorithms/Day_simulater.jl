...
function operation(kWh_cap, po_cap, Power, SoC_start, Power_rate, Connected, ac_do, ac_up, C_do, C_up)
    ...
    for m=1:M
        ...
        if sum(flex[:,1]) <= ac_do[m]*C_do[m]
            leftover[1] =  ac_do[m]*C_do[m]-sum(flex[:,1])
            Leftover_storer[m,1] = leftover[1]
        end
        if sum(flex[:,2]) <= ac_up[m]*C_up[m]
            leftover[2] =  ac_up[m]*C_up[m]-sum(flex[:,2])
            Leftover_storer[m,2] = leftover[2]
        end

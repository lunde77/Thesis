...
function load_sampling_data(Day, sampled_numbers, d_in)
    ...
    elseif Sampling == 2 # Minute-to_minute distribution sampled in non-correaltion
        # Create an array containing all possible values in the range
        start_range = 1
        end_range = 363
        all_values = collect(start_range:end_range)
        ...
        for t=1:24
            for m=1:60
                ...
                global total_flex_do_s[t,m,:] = dis[1,sampled_numbers,t,m]
                global total_flex_up_s[t,m,:] = dis[2,sampled_numbers,t,m]
                global res_20_s[t,m,:] = dis[3,sampled_numbers,t,m]
            end
        end
        OOS_numbers = 0
        sampled_numbers = 0
